...
import { Building, ArrowLeft, CheckCircle, Loader2 } from "lucide-react";
...
export default function Signup() {
  ...
  const onSubmit = async (data: SignupFormData) => {
    ...
    try {
      ...
      console.log("📧 Step 1: Creating Firebase account for:", data.adminEmail);
      ...
      console.log("✅ Step 1 complete: Firebase account created:", userCredential.user.uid);

      // Step 2: Get Firebase ID token for authenticated API calls
      ...
      // Step 4: Create the strata
      console.log("🏢 Step 3: Creating strata:", data.strataName);
      ...
      if (!strataResponse.ok) {
        ...
        console.error("❌ Strata creation failed:", errorData);

        // Account created but strata failed - redirect to dashboard where onboarding wizard will show
        toast({
      ...
      console.log("✅ Step 3 complete: Strata created:", strataData.id);

      // Store the created strata info to prevent onboarding wizard from showing
      localStorage.setItem('selectedStrata', strataData.id);
      localStorage.setItem('signupComplete', 'true');
      localStorage.setItem('needsPaymentSetup', 'true'); // Flag that payment setup is needed

      // Success! Show toast and redirect to billing for payment setup
      toast({
      ...
      // Redirect to billing page for payment setup
      console.log("🎉 Signup complete! Redirecting to payment setup...");
      setTimeout(() => {
        setLocation("/billing");
      }, 1000);
