            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 lg:gap-6">
              <div className="space-y-3">
                ...
                <div>
                  ...
                  <div className="mt-1 text-xs lg:text-sm space-y-1">
                    <p><strong>Monthly Rate:</strong> ${registration.unitCount <= 100 ? '79.95' : '129.95'}</p>
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <Select onValueChange={(value) => {
                        ...
                        const rate = value === 'standard' ? 79.95 : 
                                     value === 'premium' ? 129.95 : 0;
...
export default function Admin() {
  ...
  const [showApprovalDialog, setShowApprovalDialog] = useState(false);
  const [selectedRegistrationForApproval, setSelectedRegistrationForApproval] = useState<any>(null);
  const [activeTab, setActiveTab] = useState<"strata" | "users" | "registrations">("strata");
  ...
  const deleteStrataMutation = useMutation({
    mutationFn: async (strataId: string) => {
      const response = await fetch(`/api/admin/strata/${strataId}`, {
        method: 'DELETE',
      });
      if (!response.ok) {
        const error = await response.json();
        throw new Error(error.message || 'Failed to delete strata organization');
      }
      return response.json();
    },
    onSuccess: () => {
  ...
  const handleManageSubscription = (strata: Strata) => {
    ...
    const monthlyRate = (strata as any).subscriptionTier === 'standard' ? 79.95 : 
                       (strata as any).subscriptionTier === 'premium' ? 129.95 : 0;
    subscriptionForm.reset({
