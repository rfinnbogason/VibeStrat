  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
        ...
        <Button className="bg-green-500 hover:bg-green-600 text-white w-full sm:w-auto text-sm sm:text-base">
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              ${fundsLoading ? "..." : funds.find(f => f.type === 'reserve')?.balance ? 
                parseFloat(funds.find(f => f.type === 'reserve')!.balance).toLocaleString() : "125,000"}
            </div>
            <p className="text-xs text-muted-foreground">
              Target: ${funds.find(f => f.type === 'reserve')?.target ? 
                parseFloat(funds.find(f => f.type === 'reserve')!.target!).toLocaleString() : "150,000"}
            </p>
          </CardContent>
        </Card>
        ...
        <TabsContent value="funds" className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {funds.map((fund) => {
              ...
              const progressPercentage = target ? Math.min((balance / target) * 100, 100) : 0;
              const monthlyGrowth = fund.interestRate ? 
                (balance * parseFloat(fund.interestRate) / 100 / 12) : 0;
                          ...
                          <div className="text-sm">
                            <div className="flex justify-between items-center mb-1">
                              ...
                              <span className="font-bold text-blue-600">
                                {parseFloat(fund.interestRate).toFixed(2)}%
                              </span>
                            </div>
                            <div className="flex justify-between items-center mb-1">
