...
export default function Sidebar() {
  ...
  const getUserRole = (): UserRole => {
    ...
    console.log('🎭 Sidebar getUserRole:', { 
      userEmail: user.email, 
      userRoleData: userRoleData?.role, 
      selectedStrataId 
    });
      ...
      <div className={`p-4 m-4 bg-white border border-gray-200 rounded-lg ${isCollapsed ? 'mx-1 p-2' : ''} relative`}>
        {isCollapsed ? (
          <div className="flex justify-center">
            <img src={logoSvg} alt="VibeStrat Logo" className="h-4 w-4" />
          </div>
        ) : (
          <div className="flex items-center justify-between">
            <h1 className="flex items-center text-gray-700 font-bold text-[14px]">
              <img src={logoSvg} alt="VibeStrat Logo" className="mr-3 h-6 w-6" />
              VibeStrat
            </h1>
